from lab.project.extended_intiger_list import IntegerList
...
class TestIntegerList(TestCase):
    ...
    def test_add_integer_is_added(self):
        result = self.list_integers.add(9)
        self.assertEqual([5, 6, 7, 9], result)
    ...
    def test_remove_index_with_not_in_range_raises(self):
    ...
    def test_insert_non_integer_raises(self):
        with self.assertRaises(ValueError) as ex:
            self.list_integers.insert(0, 10.50)
        self.assertEqual('Element is not Integer', str(ex.exception))
